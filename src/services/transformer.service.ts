export default class SQLTransformer {
    ...
    private transformationMap: { [key: string]: (args?: TransformationArgs) => string } = {
      is_null: (args) => {
        if (!args || !args.column) throw new Error('Column is required for IS_NULL');
      ...
      bitwise_or: (args) => {
        if (!args || !args.column || !args.additional_input_column) {
          throw new Error('Both column and additional_input_column are required for BITWISE_OR');
        }
        return `BITWISE_OR(${args.column}, ${args.additional_input_column})`;
      },
      null_if: (args) => {
        if (!args || !args.condition || !args.valueColumnName) {
          throw new Error('Both condition and valueColumnName are required for NULL_IF');
        }
    ...
    private transformSingle(transformation: Transformation): string {
      ...
      if (!transformFunc) {
        throw new Error(`Unsupported transformation: ${transformation.transformation}`);
      }
  
      // Prepare arguments for the transformation
      const args = transformation.transformation === 'is_null' 
